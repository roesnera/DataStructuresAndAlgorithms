...
function collectOddValues(arr: Array<number>): Array<number> {
    if(arr.length===0){
        return [];
    }
    const result: Array<number> = [];
    function helper(input: Array<number>){
        if(input.length<=0){
            return;
        }
...
function collectOddValuesPure(arr: Array<number>): Array<number> {
...
function reverse(str: string): string {
    let stringToReturn: string = str.substring(str.length-1);
...
function someRecursive(arr: Array<number>, fn: Function): boolean{
    if(arr.length===1){
        return fn(arr[0]);
    }
...
function stringifyNumbers(obj: {[key:string]:any}) {
    let objToReturn: {[key:string]:any} = {};
